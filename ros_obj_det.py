...
class PeopleObjectDetectionNode(object):
    ...
    def __init__(self):
        ...
        # Get the parameters
        (model_name, num_of_classes, label_file, camera_namespace,video_name) = self.get_parameters()
        ...
        self.pub_detections = rospy.Publisher('people_object_detection/detections', DetectionArray, queue_size=1)

        # Advertise the result of Object Detector
        self.pub_detections_image = rospy.Publisher('people_object_detection/detections_image', Image, queue_size=1)
    ...
    def rgb_callback(self, data):
        ...
        try:
            ...
            # Create the message
            msg = utils.create_detection_msg(data, output_dict, category_index, self._bridge)

            # Draw bounding boxes
            image_processed = self._detector.visualize(cv_image, output_dict)

            # Convert numpy image into sensor img
            msg_im = self._bridge.cv2_to_imgmsg(image_processed, encoding="passthrough")
